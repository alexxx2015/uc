...
void processResponse(notifyResponse_ptr eventResponse, struct str_message *sys_msg, struct systrace_answer *sys_ans)
{
  ...
  eventLog("Mechanisms triggered due to event: ", eventResponse->event);
...
{
  actionDescription_ptr actionDesc=actionDescriptionFind(pdp->actionDescStore, DFM_eventnames[sys_msg->msg_data.msg_ask.code], TRUE);
  event_ptr event=eventNew(actionDesc, IDX_ONGOING, sys_msg->msg_type==SYSTR_MSG_ASK);
...
event_ptr esfParseSyscall(struct str_message *sys_msg)
{
  ...
  for(a=0; a<esfsyscalls[sys_msg->msg_data.msg_ask.code]->numParams; a++)
  {
    ...
    { // PARAM_STRING
      ...
      {
        if((IGNORE_SYS_FILES)&&(mayIgnore(filename)))
          return event;
      }
  ...
  eventLog("finished processing params: ", event);
  return event;
}
...
//    printf("%d ",sys_msg->msg_data.msg_ask.code);
  ...
  //case SYS_creat:
    ...
    case SYS_read:      // Event parameters: command user filename fd (+retval) [+buf dest addr]
      if(IGNORE_SYS_FILES && (mayIgnore(p_filename)))
      {
        actionDescription_ptr actionDesc=actionDescriptionFind(pdp->actionDescStore, DFM_eventnames[sys_msg->msg_data.msg_ask.code], TRUE);
        event=eventNew(actionDesc, IDX_ONGOING, isTry);
      }
      else
      {
    ...
    case SYS_sendmsg:   // Event parameters: command user filename filedescriptor  (+retval)
      ...
      { // TODO: this actionDescription will never be found!
        ...
        actionDescription_ptr actionDesc=actionDescriptionFind(pdp->actionDescStore, cflags, TRUE);
        event=eventNew(actionDesc, IDX_ONGOING, isTry);
      }
      else
      {
