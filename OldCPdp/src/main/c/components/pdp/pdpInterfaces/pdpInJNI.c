...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *jvm, void *reserved)
{
  ...
  jclass lclsLong=(*lenv)->FindClass(lenv, "java/lang/Long");
  ...
  if(lclsPDPinstance==NULL || lclsDecision==NULL || lclsExecuteAction==NULL || lclsAuthorizationAction==NULL ||
     lclsEvent==NULL || lclsParameter==NULL || lclsArrayList==NULL || lclsLong==NULL || lclsInteger==NULL || lclsBoolean==NULL || lclsByteArray==NULL)
  ...
  jniLong.cls=(*lenv)->NewGlobalRef(lenv, lclsLong);
  ...
  if(jniPDP.cls==NULL || jniDecision.cls==NULL || jniExecuteAction.cls==NULL || jniAuthAction.cls==NULL ||
     jniEvent.cls==NULL || jniParam.cls==NULL || jniArrayList.cls==NULL || jniLong.cls==NULL ||
     jniInteger.cls==NULL || jniBoolean.cls==NULL || jniByteArray.cls==NULL)
  ...
  (*lenv)->DeleteLocalRef(lenv, lclsLong);
  ...
  log_debug("Loading methods from Event");
...
//  jniEvent.getParams=(*lenv)->GetMethodID(lenv, jniEvent.cls, "getParams", "()Ljava/util/ArrayList;");
  jniEvent.constructor=(*lenv)->GetMethodID(lenv, jniEvent.cls, "<init>", "()V");
  jniEvent.getEventName=(*lenv)->GetMethodID(lenv, jniEvent.cls, "getEventAction", "()Ljava/lang/String;");
  ...
  jniEvent.setEventAction=(*lenv)->GetMethodID(lenv, jniEvent.cls, "setEventAction", "(Ljava/lang/String;)V");
  jniEvent.setTryEvent=(*lenv)->GetMethodID(lenv, jniEvent.cls, "setTryEvent", "(Z)V");
  jniEvent.addParam=(*lenv)->GetMethodID(lenv, jniEvent.cls, "addParam", "(Lde/fraunhofer/iese/pef/pdp/internal/Param;)V");

  log_debug("Loading methods from ArrayList");
  ...
  log_debug("Loading methods from Param");
  ...
  log_debug("Loading methods from Long");
  jniLong.constructor=(*lenv)->GetMethodID(lenv, jniLong.cls, "<init>", "(J)V");
  jniLong.longValue=(*lenv)->GetMethodID(lenv, jniLong.cls, "longValue", "()J");

  log_debug("Loading methods from Integer");
  jniInteger.constructor=(*lenv)->GetMethodID(lenv, jniInteger.cls, "<init>", "(I)V");
  ...
  log_debug("Loading methods from Boolean");
  jniBoolean.constructor=(*lenv)->GetMethodID(lenv, jniBoolean.cls, "<init>", "(Z)V");
...
jobject prepareJNIevent(JNIEnv *env, event_ptr event)
{
  checkNullPtr(event, "Could not process NULL event for prepareJNIevent?!");
  checkNullPtr(env, "JNI-Environment is NULL?!");

  jobject jevent=(*env)->NewObject(env, jniEvent.cls, jniEvent.constructor);
  if(jevent==NULL) {log_error("ERROR: Could not create event object"); return NULL;}

  jstring eventActionName=(*env)->NewStringUTF(env, event->actionDesc->actionName);
  (*env)->CallVoidMethod(env, jevent, jniEvent.setEventAction, eventActionName);
  (*env)->CallVoidMethod(env, jevent, jniEvent.setTryEvent, event->isTry);

  int a;
  GHashTableIter paramIterator;
  gpointer key, value;
  g_hash_table_iter_init(&paramIterator, event->params);

  while(g_hash_table_iter_next(&paramIterator, &key, &value))
  {
    jstring jkey=(*env)->NewStringUTF(env, key);
    eventParam_ptr curValue=value;
    jobject jval=NULL;
    jobject curParam=NULL;
    switch(curValue->paramType)
    {
      ...
      case PARAM_CONTEXT:
      case PARAM_STRING:
        jval=(*env)->NewStringUTF(env, curValue->paramValue->paramString);
        curParam=(*env)->NewObject(env, jniParam.cls, jniParam.constructor, jkey, jval, PARAM_STRING);
        break;
      case PARAM_INT:
        log_debug("processing int value for event: paramInt=[%ld]", curValue->paramValue->paramInt);
        jval=(*env)->NewObject(env, jniInteger.cls, jniInteger.constructor, curValue->paramValue->paramInt);
        curParam=(*env)->NewObject(env, jniParam.cls, jniParam.constructor, jkey, jval, PARAM_INT);
        break;
      case PARAM_LONG:
        log_debug("processing long value for event: paramLong=[%lld]", curValue->paramValue->paramLong);
        jval=(*env)->NewObject(env, jniLong.cls, jniLong.constructor, curValue->paramValue->paramLong);
        curParam=(*env)->NewObject(env, jniParam.cls, jniParam.constructor, jkey, jval, PARAM_LONG);
        break;
      case PARAM_BOOL:
        log_debug("processing bool value for event: paramBool=[%u]", curValue->paramValue->paramInt);
        jval=(*env)->NewObject(env, jniBoolean.cls, jniBoolean.constructor, curValue->paramValue->paramInt);
        curParam=(*env)->NewObject(env, jniParam.cls, jniParam.constructor, jkey, jval, PARAM_BOOL);
        break;
      default:
        log_error("Unsupported parameter type for prepareJNIevent [%u]", curValue->paramType);
        return NULL;
    }
    (*env)->CallVoidMethod(env, jevent, jniEvent.addParam, curParam);
  }

  log_debug("Creating global reference to event object");
  jobject jeventGlobal=(*env)->NewGlobalRef(env, jevent);
  (*env)->DeleteLocalRef(env, jevent);

  log_info("JNI event object successfully created");
  return jeventGlobal;
}
