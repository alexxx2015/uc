...
operator_ptr operatorNew(const char opType)
{
  ...
  switch(opType)
  {
    ...
    case STATEBASED:nop->operands=memAlloc(sizeof(oslStateBased_t));
...
uint8_t getOperatorType(const char *str)
{
  ...
  else if(!strncasecmp(str, "stateBasedFormula",17)) return STATEBASED;
...
void operatorLog(operator_ptr op)
{
  ...
  switch(op->type)
  {
    ...
    case STATEBASED:
    {
      log_debug("state based operator logging...");
      oslStateBased_ptr curOperands=op->operands;
      log_trace("     operator: %s", curOperands->operator);
