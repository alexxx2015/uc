...
operator_ptr conditionParseSubCondition(condition_ptr curCondition, xmlNodePtr node, unsigned int *curnode, mechanism_ptr curMechanism)
{
  ...
  switch(curType)
  {
    ...
    case STATEBASED:
    {
      log_debug("preparing state based operator...");
      op=operatorNew(curType);
      oslStateBased_ptr curOperands=op->operands;

      char *operatorStr=xmlGetProp(node, "operator");
      char *param1Str=xmlGetProp(node, "param1");
      char *param2Str=xmlGetProp(node, "param2");
      char *param3Str=xmlGetProp(node, "param3");

      char* operator=strdup(operatorStr);
      char* param1=strdup(param1Str);
      char* param2=NULL;
      if(param2Str!=NULL) param2=strdup(param2Str);

      char* param3=NULL;
      if(param3Str!=NULL) param3=strdup(param3Str);
      xmlFree(operatorStr);
      xmlFree(param1Str);
      if(param2Str!=NULL) xmlFree(param2Str);
      if(param3Str!=NULL) xmlFree(param3Str);

      // retrieve dataID from trigger if present there, otherwise ask PIP for new dataID
      // search param1
      eventMatch_ptr curTrigger=curMechanism->trigger;
      paramMatch_ptr paramMatch1 = (paramMatch_ptr)g_hash_table_lookup(curTrigger->matchParams, param1);
      if(paramMatch1==NULL)
      {
        log_debug("param1 not present in trigger; asking PIP");
        char *pvalue=(char*)pipInit("initialRepresentation", param1);
        // TODO free param1...
        param1=pvalue;
      }
      else
      {
        log_debug("taking dataID=[%s] of trigger-param1", paramMatch1->value->paramString);
        param1=strdup(paramMatch1->value->paramString);
      }

      if(!strncasecmp(operator, "isCombinedWith", 14) && param2!=NULL)
      {
        // search param2
        paramMatch_ptr paramMatch2 = (paramMatch_ptr)g_hash_table_lookup(curTrigger->matchParams, param2);
        if(paramMatch2==NULL)
        {
          log_debug("param2 not present in trigger; asking PIP");
          char *pvalue=(char*)pipInit("initialRepresentation", param2);
          param2=pvalue;
        }
        else
        {
          log_debug("taking dataID=[%s] of trigger-param2", paramMatch2->value->paramString);
          param2=strdup(paramMatch2->value->paramString);
        }
      }

      curOperands->operator=operator;
      curOperands->param1=param1;
      curOperands->param2=param2;
      curOperands->param3=param3;
