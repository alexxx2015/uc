...
public class CallInstanceMethodEventHandler extends CallMethodEventHandler {
	...
	protected IStatus update() {
		...
		try {
			...
			JSONArray methodArgAddressesJSON = (JSONArray) new JSONParser()
					.parse(getParameterValue("methodArgAddresses"));
			...
			// security label
			// System.out.println("CIEventHandler| ML: "+methodLabel+",
			// chopLabel: "+chopLabel);
		...
		if (isConstructor) {
			...
			// called the constructor enables to get the correct instance when
		...
		// method parameters to it: ObjectName{_name=Proc50577|java.lang.String|Thread1implicit}
		...
		//process method parameteres
		insertArguments(chopLabel.getArgs(), methodArgTypes, methodArgAddresses, pid, threadId, parentClass,
				parentObjectAddress, parentMethod, calleeObjectClass, calleeObjectAddress, calledMethod,
