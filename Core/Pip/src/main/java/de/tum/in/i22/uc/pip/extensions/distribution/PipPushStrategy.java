...
public class PipPushStrategy extends PipDistributionStrategy{
	...
	public IStatus doRemoteEventUpdate(Location location, IEvent event) {
		...
		try {
			Any2PipClient pipHandle = _connectionManager.obtain(_clientHandlerFactory.createPipClientHandler(location));
	...
	public IStatus doRemoteDataFlow(Location location, Map<IName, Set<IData>> dataflow) {
		...
		try {
			Any2PipClient _pipHandle = _connectionManager.obtain(_clientHandlerFactory.createPipClientHandler(location));
