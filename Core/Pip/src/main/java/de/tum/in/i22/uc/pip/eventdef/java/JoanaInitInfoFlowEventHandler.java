...
public class JoanaInitInfoFlowEventHandler extends BaseEventHandler {
	...
	protected IStatus update() {
		...
		//Version 3: signature+location+parampos is used as naming identifier for a container
		//Version 4: signature+parampos is used as naming identifier for a container
		String[] infoConts = new String[]{signature
										, location+_javaIFDelim+offset+_javaIFDelim+signature
										, location+_javaIFDelim+offset+_javaIFDelim+signature+_javaIFDelim+parampos
...
//		Process alias relationship
		IContainer sig = _informationFlowModel.getContainer(new NameBasic(prefix+infoConts[0]));
		IContainer locSig = _informationFlowModel.getContainer(new NameBasic(prefix+infoConts[1]));
		IContainer locSigPar = _informationFlowModel.getContainer(new NameBasic(prefix+infoConts[2]));
		IContainer sigPar = _informationFlowModel.getContainer(new NameBasic(prefix+infoConts[3]));
