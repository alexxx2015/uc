...
import de.tum.in.i22.uc.pmp.xsd.SinceType;
...
public class Since extends SinceType
{
  ...
  { // A occurs, SINCE is satisfied (LTL doesn't state anything about B in the timestep when A happens)
    ...
    if(curEvent==null)
    {
      ...
      {
        if(!this.state.immutable)
        { // immutable until next occurence of subformula A
      ...
      if(!this.state.subEverTrue && !operand2state)
      {
        ...
        this.state.subEverTrue=true; // intention here subformula was ever FALSE (in contrast to name...)
        this.state.immutable=true;
      }
    }
