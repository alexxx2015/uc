...
public class ContextMenu extends PopupMenu implements ActionListener {
    ...
    //My addition, 10 March 2013. For param deletion    
    private static ContextMenu removeParamMenu = new ContextMenu();
    private static MenuItem removeParamItem;
    private final static String REMOVE_PARAM_BLOCK = "REMOVEPARAM";
    private static boolean removeParamMenuInit = false;
    
    private final static String ADD_PARAM_BLOCK = "ADDPARAM";
     ...
     * 13 March 2013.
     */
    private static void initRemoveParamMenu(){
    	removeParamItem=new MenuItem("Delete Param");
    	removeParamItem.setActionCommand(REMOVE_PARAM_BLOCK);
    	removeParamItem.addActionListener(rndBlockMenu);
    	removeParamMenu.add(removeParamItem);
    	removeParamMenuInit=true;
    }
    ...
    public static PopupMenu getContextMenuFor(Object o) {
        if (o instanceof RenderableBlock) {
            ...
            if (((RenderableBlock) o).hasParam()) {
                if (!removeParamMenuInit) {
                    initRemoveParamMenu();
                }
                activeComponent = o;
                return removeParamMenu;
            }
            /*else {
    ...
    public void actionPerformed(ActionEvent a) {
        ...
        } else if (a.getActionCommand() == ADD_PARAM_BLOCK) {
            //notify the renderableblock componenet that lauched the conetxt menu
            if (activeComponent != null && activeComponent instanceof RenderableBlock) {
                ((RenderableBlock) activeComponent).addParam();
            }
        ...
        else if (a.getActionCommand() == REMOVE_PARAM_BLOCK) {
            //notify the renderableblock componenet that lauched the conetxt menu
            if (activeComponent != null && activeComponent instanceof RenderableBlock) {
            	RenderableBlock rb=(RenderableBlock) activeComponent;
            	//System.out.println("Initial Label: "+rb.getBlock().);
                rb.removeParam();
            }
        }
    }
