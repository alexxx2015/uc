...
import de.tum.in.i22.uc.blocks.codeblockutil.LabelWidget;
...
public class BlockLabel implements MouseListener, MouseMotionListener, KeyListener {
    ...
    public final static Font blockFontSmall_Bold = new Font("Arial", Font.BOLD, 10);

    public final static Font blockFontMedium_Bold = new Font("Arial", Font.BOLD, 13);

    public final static Font blockFontLarge_Bold = new Font("Arial", Font.BOLD, 15);

    public final static Font blockFontSmall_Plain = new Font("Arial", Font.PLAIN, 10);

    public final static Font blockFontMedium_Plain = new Font("Arial", Font.PLAIN, 13);

    public final static Font blockFontLarge_Plain = new Font("Arial", Font.PLAIN, 15);
    ...
    protected void textChanged(String text) {
    	//Get previous text packaged so our event listener will have some easy task    	
    	PreviousState ps=new PreviousState(workspace.getEnv().getBlock(blockID).getBlockLabel());
        ...
        if ((this.labelType.equals(BlockLabel.Type.NAME_LABEL) || this.labelType.equals(BlockLabel.Type.PORT_LABEL))
            ...
            if (rb != null) {             	
                ...
                workspace.notifyListeners(new WorkspaceEvent(workspace, rb.getParentWidget(), blockID, WorkspaceEvent.BLOCK_RENAMED, true, ps));
            }
        }
    }
    ...
    protected void genusChanged(String genus) {
    	...
    	//Get old state of this block label
    	Block oldBlock = workspace.getEnv().getBlock(blockID);
    	PreviousState psObject=new PreviousState(oldBlock.getGenusName());
    	
    	//15 March 2013.
    	if(oldBlock.getOriginalGenusName().equals("")) oldBlock.setOriginalGenusName(oldBlock.getGenusName());  
    	
        if (widget.hasSiblings()) {
            //Block oldBlock = workspace.getEnv().getBlock(blockID);
            oldBlock = workspace.getEnv().getBlock(blockID);
            ...
            workspace.notifyListeners(new WorkspaceEvent(workspace, rb.getParentWidget(), blockID, WorkspaceEvent.BLOCK_GENUS_CHANGED, psObject));
        }
    }
