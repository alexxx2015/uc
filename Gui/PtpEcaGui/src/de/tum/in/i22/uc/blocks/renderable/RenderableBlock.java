...
import de.tum.in.i22.uc.blocks.codeblocks.BlockShape;
import de.tum.in.i22.uc.blocks.codeblocks.InfixBlockShape;
import de.tum.in.i22.uc.blocks.codeblocks.JComponentDragHandler;
import de.tum.in.i22.uc.blocks.codeblocks.rendering.BlockShapeUtil;
import de.tum.in.i22.uc.blocks.codeblockutil.CToolTip;
import de.tum.in.i22.uc.blocks.codeblockutil.GraphicsManager;
import de.tum.in.i22.uc.blocks.renderable.JTableX;
import de.tum.in.i22.uc.blocks.renderable.Param;
import de.tum.in.i22.uc.blocks.renderable.ParamLabel;
import de.tum.in.i22.uc.blocks.renderable.BlockImageIcon.ImageLocation;
import de.tum.in.i22.uc.blocks.workspace.ContextMenu;
import de.tum.in.i22.uc.blocks.workspace.FactoryManager;
import de.tum.in.i22.uc.blocks.workspace.ISupportMemento;
import de.tum.in.i22.uc.blocks.workspace.MiniMap;
import de.tum.in.i22.uc.blocks.workspace.Page;
import de.tum.in.i22.uc.blocks.workspace.RBParent;
import de.tum.in.i22.uc.blocks.workspace.SearchableElement;
 ...
 */
public class RenderableBlock extends JComponent implements SearchableElement, MouseListener, MouseMotionListener, ISupportMemento, CommentSource, ParamSource {
    ...
    /** The param of this.  May be null */
	private Param param=null;
	/**  set true when comment is added or removed from this block */
	private boolean paramLabelChanged = false;
    ////////////////////////
    //ATTRIBUTE FIELDS
    ...
    //My addition, 25 February 2013
    private PreviousState oPreviousState;    
    ...
    public void setLocation(int x, int y) {
        ...
        if (hasParam() && !(dx == x && dy == y )) {
			if (getParam().getParent() != getParent()) {
				getParam().setParent(getParent(), Workspace.DRAGGED_BLOCK_LAYER);
			}
			getParam().translatePosition(dx, dy);
		}
    }
    ...
    public void addComment() {
        addCommentProperly(true);
    }
    
    public void addCommentProperly(boolean fireEvent){
    	 if (hasComment()) {
             //a renderable block may only have ONE comment
         } else {
             int x = this.getX() + this.getWidth() + 30;
             int y = this.getY() - 40;
             comment = new Comment(workspace, "", this, this.getBlock().getColor(), zoom,fireEvent);
...
////////////////////////
/**
* @return true iff this.param !=null
*/
...
/**
* Sets this RenderableBlock's param
* @param param
*/
...
this.param = param;
	...
	/**
	 * Add a new param to this parent Container at a point (10,-20) away from
	 * upper-right hand corner. Modify such that this.hasComment will now return
	 * true.
	 */
	public void addParam() {
		addParamProperly(true);
	}
	
	public void addParamProperly(boolean fireEvent){
		// getUserParam(this.getGenus()) ;

				if (hasParam()) {
					// if param already exists, it must be redrawn
					removeParam();
					addParam();
					...
					int x = this.getX() + this.getWidth() + 30;
					int y = this.getY() - 40;
					param = new Param(workspace, this.getGenus(), this, this.getBlock().getColor(), zoom, fireEvent);
					// param.setBackground(this.getBLockColor());
					if (this.getParentWidget() != null) {
						param.setParent(this.getParentWidget().getJComponent());
					} else {
						param.setParent(this.getParent());
					}
					param.setLocation(x, y);
					paramLabelChanged = true;
				}
				revalidate();
				getHighlightHandler().revalidate();
				updateBuffImg();
				param.getArrow().updateArrow();
				getParent().repaint();
	}
    ...
    public void removeCommentProperly(boolean fireEvent){
    	if (hasComment()) {
            comment.deleteProperly(fireEvent);
            comment = null;
            commentLabelChanged = true;
            reformBlockShape();
            revalidate();
    ...
    /**
	 * remove this param from this parent Container and modify such that
	 * this.hasParam returns false.
	 */
	public void removeParam(){
		removeParamProperly(true);
	}
	
	public void removeParamProperly(boolean fireEvent){
		if(hasParam()){
			param.deleteProperly(fireEvent);
			param = null;
			paramLabelChanged = true;		
			reformBlockShape();
			revalidate();
	...
	public Point getParamLocation() {
		...
		if (param != null) {
           ParamLabel paramLabel = param.getParamLabel();
			if (paramLabel != null) {
				location.translate(paramLabel.getX() - 2, paramLabel.getY() - 2);
				location.translate(paramLabel.getWidth()/2, paramLabel.getHeight()/2);
			}
		}
    ...
    private void startDragging(RenderableBlock renderable, WorkspaceWidget widget) {
        ...
        if(renderable.hasParam()){
    		renderable.param.setConstrainParam(false);
    	}
    ...
    public static void stopDragging(RenderableBlock renderable, WorkspaceWidget widget) {
      ...
      //move param
        if(renderable.hasParam()){
    		if(renderable.getParentWidget() !=null){
    			renderable.param.setParent(renderable.getParentWidget().getJComponent(),0);
    		}else{
    			renderable.param.setParent(null, renderable.getBounds());
    		}
    		
    		renderable.param.setConstrainParam(true);
    		renderable.param.setLocation(renderable.param.getLocation());
    		renderable.param.getArrow().updateArrow();
    	}
    }
        ...
        if (SwingUtilities.isLeftMouseButton(e)) {
            if (pickedUp) {
                ...
                if (dragging) {
                    ...
                    /*
                    WorkspaceWidget w=workspace.getWidgetAt(workspace.getTrashCanWidget().getJComponent().getLocation());
                    System.out.println("Widget is contained at: "+w.getJComponent().getLocation());
                    System.out.println("Event at: "+e.getLocationOnScreen());
                    Point p=SwingUtilities.convertPoint((JComponent)workspace, e., w.getJComponent());
                    System.out.println("Widget contains: "+w.contains(p.x, p.y));*/
                    //workspace.notifyListeners(new WorkspaceEvent(workspace, widget, link, WorkspaceEvent.BLOCK_MOVED, true));
                    workspace.notifyListeners(new WorkspaceEvent(workspace, widget, this.blockID, WorkspaceEvent.BLOCK_MOVED, true,oPreviousState));
    ...
    public void mousePressed(MouseEvent e) { 
    	oPreviousState=new PreviousState(getLocation());
        if (SwingUtilities.isLeftMouseButton(e)) {
    ...
    public Node getSaveNode(Document document) {
    	...
    	return getBlock().getSaveNode(document, descale(this.getX()), descale(this.getY()),
                    param != null ? param.getSaveNode(document) : null,comment != null ? comment.getSaveNode(document) : null, isCollapsed());
        ...
        if (isBlock || isBlockStub) {
            ...
            for (int i = 0; i < children.getLength(); i++) {
                ...
                if (child.getNodeName().equals("Location")) {
					...
					rb.param = Param.loadParam(workspace,child.getChildNodes(), rb);					
					if (rb.param != null) {
						rb.param.setParent(rb.getParentWidget().getJComponent());
					}
				}                
