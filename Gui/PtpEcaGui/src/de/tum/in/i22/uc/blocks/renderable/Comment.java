...
public class Comment extends JPanel {
    ...
    //09 March 2013
    private boolean bFireEvent=true;
    ...
    public Comment(Workspace workspace, String initText, CommentSource source, Color borderColor, double zoom) {
        this(workspace,initText,source,borderColor,zoom,true);
    }
    
    public Comment(Workspace workspace, String initText, CommentSource source, Color borderColor, double zoom, boolean fireEvent){
        ...
        textArea.getDocument().addDocumentListener(new DocumentListener(){
			...
			private PreviousState prepareState(String s){				
				PreviousState ps=new PreviousState(Comment.this.commentSource);
				ps.addOtherStates(sOldCommentText);
				ps.addOtherStates(s);				
				return ps;
			}
			...
			public void insertUpdate(DocumentEvent e) {
				...
				if(bFireEvent)
				Comment.this.workspace.notifyListeners(new WorkspaceEvent(Comment.this.workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_CHANGED,prepareState("insert")));					
			}

			@Override
			public void removeUpdate(DocumentEvent e) {
				//Have to find out why I cannot get the character removed here.
				if(bFireEvent)
					Comment.this.workspace.notifyListeners(new WorkspaceEvent(Comment.this.workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_CHANGED,prepareState("delete")));
			}
        	
        });
        
        //This is helpful during deletion of event. It gets called before document
        //listener removal above, so we can get original text here
        textArea.addKeyListener(new KeyAdapter() {
            public void keyPressed(KeyEvent e) {            	
            	sOldCommentText=getText();
            }            	            	            
        });
        ...
        if(fireEvent)
        workspace.notifyListeners(new WorkspaceEvent(workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_ADDED,psObject));
        
        //init
        pointOld=new Point();
        dimensionOld=new Dimension();
    }
    ...
    public void deleteProperly(boolean fireEvent){
    	...
    	PreviousState psObject=new PreviousState(commentSource);
    	psObject.addOtherStates(this.getText());
    	...
    	if(fireEvent)
    	workspace.notifyListeners(new WorkspaceEvent(workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_REMOVED,psObject));
    ...
    public void setText(String text) {
        setTextProperly(text,true);        
    }
    
    public void setTextProperly(String text, boolean fireEvent){
    ...
    private class CommentEventListener implements FocusListener, MouseListener, MouseMotionListener {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            Comment.this.pointOld=Comment.this.getLocation();
            Comment.this.dimensionOld=Comment.this.getSize();
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            if(Comment.this.isResizing()){
            	...
            	ps=new PreviousState(Comment.this);
            	ps.addOtherStates(Comment.this.getCommentSource());
            	ps.addOtherStates(Comment.this.dimensionOld);
            	workspace.notifyListeners(new WorkspaceEvent(workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_RESIZED,ps));
            }
            else{
            	//means this comment might have been moved. We can fire event here
            	ps=new PreviousState(Comment.this);
            	ps.addOtherStates(Comment.this.getCommentSource());
            	ps.addOtherStates(Comment.this.pointOld);
            	workspace.notifyListeners(new WorkspaceEvent(workspace, getCommentSource().getParentWidget(), WorkspaceEvent.BLOCK_COMMENT_MOVED,ps));
            }
            setResizing(false);
            setPressed(false);
            repaint();
