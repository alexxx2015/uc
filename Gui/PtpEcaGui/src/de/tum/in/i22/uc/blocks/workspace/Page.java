...
public class Page implements WorkspaceWidget, SearchableContainer, ISupportMemento {
    ...
    public void clearPage() {
        for (RenderableBlock block : this.getBlocks()) {
        	if(block.hasParam()) block.removeParam();
        	if(block.hasComment()) block.removeComment();
    ...
    public void addBlock(RenderableBlock block) {    	
    	//My addition. Get the old location of this block
    	//Point oldPoint=block.getLocation();
        ...
        if (oldParent != this) {        	
            //workspace.notifyListeners(new WorkspaceEvent(workspace, oldParent, Block.NULL, WorkspaceEvent.BLOCK_MOVED));
    ...
    /* 22 Feb 2013
     * Initially when I call clearPage(), the environment is not emptied
    *  this procedure ensures this objective
    */
