package de.tum.in.i22.uc.blocks.codeblocks;
...
import de.tum.in.i22.uc.blocks.codeblocks.BlockConnector.PositionType;
import de.tum.in.i22.uc.blocks.renderable.BlockImageIcon;
...
import de.tum.in.i22.uc.blocks.workspace.WorkspaceEnvironment;
import de.tum.in.i22.uc.utilities.PublicMethods;
import de.tum.in.i22.uc.utilities.PublicVariable;
...
public class Block implements ISupportMemento {
    ...
    //15 March 2013. Some fine tuning. When a block's genus name is changed,
    //a parameter is created. When this parameter is deleted, the block genus name should
    //revert to original name.
    private String sOriginalGenusName="";
    ...
    public String getOriginalGenusName(){
    	return sOriginalGenusName;
    }
    
    public void setOriginalGenusName(String s){
    	sOriginalGenusName=s;
    }
    ...
    public void changeGenusTo(String genusName) {    	    	
    	...
    	// a flag to track if the touched block was action
    	PublicVariable.calledAfterAction = false; 
        ...
        java.util.List<String> actionList = new ArrayList<String>();
		try {
			actionList = new XMLcombinedmodel().actionName;
		} catch (ParserConfigurationException e) {
			e.printStackTrace();
		} catch (TransformerException e) {
			e.printStackTrace();
		}
        boolean isAction = new PublicMethods().stringIsInList(genusName, actionList);
        if(isAction){ 
        	PublicVariable.calledAfterAction = true;
        	PublicVariable.action=genusName; 
        	//set parameters only when their blocks exist

//        	if(actionList.contains(genusName))
        	workspace.getEnv().getRenderableBlock(blockID).addParam();
        		//comment setparameters function as of now, not needed
//        		setParameters(savestr);
         } else System.out.println("block touched in not an action");
        // end of modified by Prachi
     ...
     */
    public Node getSaveNode(Document document, int x, int y, Node paramNode, Node commentNode, boolean isCollapsed) {
        ...
        if (paramNode != null) {
        	blockElement.appendChild(paramNode);
        }
