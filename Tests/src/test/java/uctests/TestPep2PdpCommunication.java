...
public class TestPep2PdpCommunication extends GenericTest{
	...
	public void testNotifyStarEvent() throws Exception {
		...
		// check if status is not null
		Assert.assertEquals(EStatus.INHIBIT, response1.getAuthorizationAction().getEStatus());
		Assert.assertEquals(EStatus.INHIBIT, response2.getAuthorizationAction().getEStatus());
	}
