...
public class StateBasedOperatorTest {
	...
	public void testIsNotIn() throws Exception {
		...
		//0: deploy policy
		String path = StateBasedOperatorTest.class.getResource("/testStateBasedOperators.xml").getPath();
		_logger.debug("deploying policy " + path+"...");
		_logger.debug("deploying policy " + path+" result : "+_pmp.deployPolicyURIPmp(path));
		...
		Assert.assertEquals(EStatus.ALLOW, response1.getAuthorizationAction().getEStatus());
		
		// artificially add the same data to secondContainer
		_pip.initialRepresentation(new NameBasic ("secondContainer"),_pip.getDataInContainer(new NameBasic("initialContainer")));
		
		// 2: send the event. This time param name and parm value match but condition is false
		IResponse response2 = _pdp.notifyEventSync(event);
		_logger.debug("2: Received response as reply to " + event + ":" + response2);
		Assert.assertNotNull(response2);
		Assert.assertEquals(EStatus.ALLOW, response2.getAuthorizationAction().getEStatus());
		
		//deploy semantics for a new event "testEvent" that empties initialcontainer
		File file = FileUtils.getFile("src/test/resources/updateIF.jar");
		_logger.debug("deployment of new semantics....: "+_pip.updateInformationFlowSemantics(null, file, EConflictResolution.OVERWRITE));
				
		//send "testevent". set appropriate pep in order to match event definition package.
		Map<String,String> dm= createDummyMap();
		dm.put(Settings.getInstance().getPepParameterKey(), "test");
		IResponse response3 = _pdp.notifyEventSync(mf.createActualEvent("TestEvent", dm));
		_logger.debug("2.5: Received response as reply to testEvent event:" + response3);
		Assert.assertNotNull(response3);
		Assert.assertEquals(EStatus.ALLOW, response3.getAuthorizationAction().getEStatus());
		
		// 3: send the event. This time param name and parm value match and condition should be true. thus the event should be inhibited
		IResponse response4 = _pdp.notifyEventSync(event);
		_logger.debug("4: Received response as reply to " + event + ":" + response4);
		Assert.assertNotNull(response4);
		Assert.assertEquals(EStatus.INHIBIT, response4.getAuthorizationAction().getEStatus());
