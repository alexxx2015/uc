...
{ //by equals we mean "with the same ID"
  int i=0;
...
unsigned int cont_free(cont_ptr data)
{
  /** TODO **/
}
...
{ //by equals we mean "with the same ID". we don't look into the list of contained.
  int i=0;
  ...
  while((cont1->cont_ID[i] == cont2->cont_ID[i]) && (cont1->cont_ID[i] != '\0')
      && (cont2->cont_ID[i] != '\0'))
  {
    i++;
  }
...
unsigned int list_of_data_add(list_of_data_ptr* list_of_data, data_ptr data)
{
  ...
  if(ptr != NULL)
  { //if list is not empty, checks for duplicates
...
unsigned int list_of_cont_add(list_of_cont_ptr *list_of_cont, cont_ptr cont)
{
  ...
  { //if list is not empty
    ...
    { //check if cont is the first element
...
alias_ptr alias_copy(alias_ptr alias)
{
  ...
  while(tmp_source != NULL)
  {
    ...
    {
      tmp_dest->aliases[x]=(char*)malloc(sizeof(char)
          * (strlen(tmp_source->aliases[x])));
...
unsigned int alias_add(alias_ptr* alias, char* from, char* to)
{
  ...
  if((*alias) == NULL)
  { //emty list -> insert the new value at the beginning
  ...
  else
  {
    ...
    if((tmp->next == NULL) && (strcmp(tmp->cont_ID, from) != 0))
    { //no matchings for "FROM" -> add a new node at the end
      tmp->next=alias_new();
      (tmp->next)->size=1;
    ...
    { //there's a mathing for "FROM" and it's pointed by tmp
      //let's look for "TO"
      x=0;
      while(x < (tmp->size))
      {
...
unsigned int alias_remove(alias_ptr* alias, char* from, char* to)
{
  ...
  if(strcmp(from, (*alias)->cont_ID) == 0)
  { //The "FROM" is the first element
    if((*alias)->size == 0) return 1; //no aliases for "FROM"
    x=0;
    while(x < ((*alias)->size))
    {
      if(strcmp((*alias)->aliases[x], to) == 0)
  ...
  { //the "FROM" is not the first of the list
    tmpprev=NULL;
    while((tmp->next != NULL) && (strcmp(tmp->cont_ID, from) != 0))
    {
      tmpprev=tmp;
...
char** alias_closure(alias_ptr alias, char* cont, int* size)
{
  ...
  while((count < (pow(10, MAX_CONT_ID))) && (changed != 0))
  {
    changed=0;
    tmpsource=alias;
    while(tmpsource != NULL)
...
// TODO: you should copy the null-terminator as well or better reserve enough memory for it!!!!!!
// otherwise you will get sth. like: FNAME x contacts.dataRD1���� -> C-FILE00000003 if the character strlen+1 changes!
unsigned int naming_add(naming_ptr* naming, char* pid, char* id, char* cont)
{
  ...
  if((*naming) == NULL)
  { //empty list -> insert the new value at the beginning
    (*naming)=naming_new();
    tmp=*naming;
    tmp->cont_ID=(char*)malloc(sizeof(char) * (strlen(cont)+1));
    strcpy(tmp->cont_ID, cont);
    tmp->id=(char*)malloc(sizeof(char) * (strlen(id)+1));
    strcpy(tmp->id, id);
    tmp->pid=(char*)malloc(sizeof(char) * (strlen(pid)+1));
    strcpy(tmp->pid, pid);
    return 0;
  }
  else
  {
    ...
    while((tmp->next != NULL) && ((strcmp(tmp->pid, pid) != 0)
        || (strcmp(tmp->id, id) != 0)))
    {
      tmp=tmp->next;
    }

    if((tmp->next == NULL) && ((strcmp(tmp->pid, pid) != 0)
        || (strcmp(tmp->id, id) != 0)))
    { //no matchings for "PID && ID" -> add a new node at the end
      tmp->next=naming_new();
      tmp=tmp->next;
      tmp->cont_ID=(char*)malloc(sizeof(char) * (strlen(cont)+1));
      strcpy(tmp->cont_ID, cont);
    ...
    { //there's a matching for "pid && id" -> overwrite the container ID
...
unsigned int naming_remove(naming_ptr* naming, char* pid, char* id)
{
  ...
  if((strcmp(tmp->pid, pid) == 0) && ((strcmp(tmp->id, id) == 0) || star))
  { //first element
  ...
  else
  {
    ...
    if((strcmp(tmp->pid, pid) == 0) && ((strcmp(tmp->id, id) == 0) || star))
    { //current element (eventually the last one) matches pid && id
    ...
    { //there's no matching for "pid && id"
      return 1;
    }
...
char* naming_find(naming_ptr naming, char* pid, char* id)
{
  ...
  if((strcmp(tmp->pid, pid) == 0) && (strcmp(tmp->id, id) == 0))
  { //current element (eventually the last one) matches pid && id
    return tmp->cont_ID;
  }
  else
...
char* dataCont_getNewContID(dataCont_ptr dataCont)
{
  ...
  for(i=0; i < MAX_CONT_ID; i++)
    id[i]=dataCont->cont_ID_counter[i];

  dataCont->cont_ID_counter[MAX_CONT_ID - 2]++; /**TODO**///MAX NUM OF CONTAINERS = 100; ***********************************************

  if(dataCont->cont_ID_counter[MAX_CONT_ID - 2] - '0' == 10)
  {
    dataCont->cont_ID_counter[MAX_CONT_ID - 2]='0';
    dataCont->cont_ID_counter[MAX_CONT_ID - 3]++;
    if(dataCont->cont_ID_counter[MAX_CONT_ID - 3] - '0' == 10)
    {
      dataCont->cont_ID_counter[MAX_CONT_ID - 3]='0';
      dataCont->cont_ID_counter[MAX_CONT_ID - 4]++;
    }
  }
...
char* dataCont_getNewDataID(dataCont_ptr dataCont)
{
	...
	return qdftGet_new_data(1, INIT_DATA_MODE);

/****OLD CODE (NOT EXECUTED) ****/
  int i;
  char* id=(char *)malloc(sizeof(char) * MAX_DATA_ID);

  for(i=0; i < MAX_DATA_ID; i++)
    id[i]=dataCont->data_ID_counter[i];

  dataCont->data_ID_counter[MAX_CONT_ID - 2]++; /**TODO**///MAX NUM OF CONTAINERS = 100; ***********************************************

  if(dataCont->data_ID_counter[MAX_DATA_ID - 2] - '0' == 10)
  {
    dataCont->data_ID_counter[MAX_DATA_ID - 2]='0';
    dataCont->data_ID_counter[MAX_DATA_ID - 3]++;
  }
  return id;
/*******************************/
...
unsigned int dataCont_removeCont(dataCont_ptr dataCont, char* cont)
{
  ...
  loc=list_of_cont_find(dataCont->cont, cont);

  if(loc == NULL) return 1; //element is not in the list

  return list_of_cont_remove(&(dataCont->cont), loc);
...
char** dataCont_getAliasesOf(dataCont_ptr dataCont, char* cont, int *size)
{
  ...
  // rd mod
  //	char **rdtmp=NULL;
  //	rdtmp=alias_closure(dataCont->alias, cont, size);
  //	if(rdtmp==NULL)
  //  {
  //  //  *size=0;
  //    printf("getting empty alias closure! -> setting size from [%d] to zero ([%d])\n",*size, *size);
  //  }
  //	return rdtmp;
...
unsigned int dataCont_updateAlias(dataCont_ptr dataCont)
{
  ...
  while((res == 0) && (loop <= pow(10, MAX_CONT_ID - 2)))
  { //MAX NUM OF ITERATIONS = NUM OF CONTAINERS
    res=1;
    tmp=dataCont->alias;
    while(tmp != NULL)
    {
      ...
      for(x=0; x < tmp->size; x++)
      {
        ...
        while(tmplod != NULL)
        {
          ...
          tmpres
              =(dataCont_addDataCont_noUpdate(dataCont, tmplod->data->data_ID, tmp->aliases[x]));
          res=(res) && (tmpres);
          tmplod=tmplod->next;
        }
        //			printf ("\n");
...
void alias_print(alias_ptr alias, int mode)
{ //only mode=1 so far /** TODO **/
  alias_ptr tmp=alias;
  int x;
...
void alias_printf(FILE *file, alias_ptr alias, int mode)
{ //only mode=1 so far /** TODO **/
  alias_ptr tmp=alias;
  int x;
